...
export function HistoryTools({ state, dispatch, selectedOid }: {
  ...
  const moveLabel = branch ? `move branch ${branch} to` : 'move HEAD to';
    ...
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 space-y-3">
      ...
      <div>
        ...
        <div className="flex gap-1">
          <button className="rounded border border-zinc-700 px-2 py-0.5 text-xs hover:bg-zinc-800 disabled:opacity-40" disabled={unborn} onClick={() => reset('soft')} title="keep index + working tree">soft</button>
          <button className="rounded border border-zinc-700 px-2 py-0.5 text-xs hover:bg-zinc-800 disabled:opacity-40" disabled={unborn} onClick={() => reset('mixed')} title="reset index, keep working tree">mixed</button>
          <button className="rounded border border-rose-800 text-rose-300 px-2 py-0.5 text-xs hover:bg-rose-950 disabled:opacity-40" disabled={unborn} onClick={() => reset('hard')} title="discard index + working tree">hard</button>
        </div>
      </div>

      {selectedOid && (
        <div>
          <div className="text-[11px] text-zinc-500 mb-1">visit a commit with no branch</div>
          <button
          ...
          >
            checkout {targetLabel} (detached)
          </button>
        </div>
      )}

      <div>
        <div className="text-[11px] text-zinc-500 mb-1">discard uncommitted changes ({recoverable.length})</div>
        <button
