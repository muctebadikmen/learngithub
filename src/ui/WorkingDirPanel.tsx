...
export function WorkingDirPanel({ state, dispatch }: { state: RepoState; dispatch: (a: GitAction) => unknown }) {
  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
      <h2 className="text-xs font-semibold tracking-wide text-zinc-500 mb-2">{t('panel.workingDir')}</h2>
        ...
        {files.map((f) => (
          <li key={f.path} className="flex items-center justify-between gap-2 text-sm font-mono">
            ...
            <span className="flex items-center gap-2">
              <span className="text-[10px] text-zinc-600">{t(`status.${f.status}` as MessageKey)}</span>
