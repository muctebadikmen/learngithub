...
export function LevelPanel({
  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
      ...
      <div className="text-sm font-medium text-zinc-100 mb-1">{t(level.title as MessageKey)}</div>
      <p className="text-sm text-zinc-400 mb-3">{t(level.goal as MessageKey)}</p>
        ...
        {level.checks.map((c, i) => {
          ...
          return (
            <li key={i} className={`flex items-center gap-2 text-sm ${ok ? 'text-emerald-400' : 'text-zinc-500'}`}>
              ...
              <span>{t(c.label as MessageKey)}</span>
            </li>
          );
        })}
      </ul>
      ...
      {complete && (
        <div className="rounded-md border border-emerald-800 bg-emerald-950/50 p-2 mb-3">
          <div className="text-sm text-emerald-300 mb-2">{t('level.complete')}</div>
          {index + 1 < total
            ? <button className="w-full rounded bg-emerald-700 px-2 py-1 text-sm hover:bg-emerald-600" onClick={onNext}>{t('level.next')}</button>
            : <div className="text-sm text-emerald-300">{t('level.finishedAll')}</div>}
        </div>
      )}

      <div className="flex flex-wrap gap-1" role="group" aria-label={t('level.levelsAria')}>
        {Array.from({ length: total }, (_, i) => {
          ...
          return (
            <button
              ...
              title={locked ? t('level.locked') : undefined}
            >
              {i + 1}
            </button>
          );
